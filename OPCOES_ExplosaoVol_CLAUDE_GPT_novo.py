...
# ------------------------------------------------------------
# Variáveis externas à função detectar_squeeze() para permitir
# testar diferentes configurações sem alterar o código interno.
# Todas são usadas como parâmetros default de detectar_squeeze(),
# ou seja, também podem ser sobrescritas na própria chamada da
# função, ex.: detectar_squeeze(df, adx_maximo=20)
# ============================================================
...
# --- BOLLINGER WIDTH ------------------------------------------
BB_WIDTH_PERCENTIL = 0.20   # percentil máximo de BB Width para squeeze (mínima)
BB_WIDTH_JANELA    = 252    # janela (em candles) do rolling do percentil

# --- ADX --------------------------------------------------------
ADX_MAXIMO = 25   # ADX máximo para squeeze (trend fraco / mínima)
...
def detectar_squeeze(
    ...
    # ==========================================================
    # NOVA REGRA DE SQUEEZE
    # ==========================================================
    out["squeeze"] = (
    
        (out["VOL_21"] <= out["VOL21_P20"])
        &
